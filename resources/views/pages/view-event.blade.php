...
@section('descriptions'){!! trim(iconv_substr(str_replace($find, '', strip_tags($event->content)), 0, 400, 'UTF-8')) . '...' !!}@endsection
@section('image', Storage::url($event->img))
@section('content')
    ...
    <div class="bg-alpha border-secondary text-light rounded border p-2">
        <h3 class="thai text-warning">กิจกรรมอื่นๆ</h3>
        <ul class="list-unstyled f-12 tahoma">
            @foreach ($events as $item)
                <li class="border-bottom border-secondary mb-1">
                    <a class="text-light news-hover hvr-icon-forward d-block" href="{{ route('view-event', $item->id) }}">
